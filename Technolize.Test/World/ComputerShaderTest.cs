...
";
        ...
        Rlgl.EnableShader(compProgram);
        Rlgl.BindShaderBuffer(outSsbo, 0);
        ...
        Console.WriteLine($"uSize location: {uLoc}");
    ...
    public unsafe void CanGetComputeShaderUniform()
    {
        ...
        // Create a tiny SSBO
        uint ssbo = Rlgl.LoadShaderBuffer(ssboSize, null, Rlgl.DYNAMIC_COPY);
    ...
    void main() {
        // Write uniform to buffer to make it live
        outData[0] = uint(uSize.x + uSize.y);
    }
    ";
        ...
        // Enable shader and bind SSBO
        Rlgl.EnableShader(compProgram);
        Rlgl.BindShaderBuffer(ssbo, 0);
        ...
        Console.WriteLine($"uSize location: {uLoc}");
        Assert.That(uLoc, Is.GreaterThanOrEqualTo(0), "Uniform uSize should have a valid location");

        // Set uniform
        int[] value = [ 42, 99 ];
        fixed (int* p = value)
            Rlgl.SetUniform(uLoc, p, (int) ShaderUniformDataType.Vec2, 1);

        // Dispatch one workgroup
        Rlgl.ComputeShaderDispatch(1, 1, 1);
        ...
        Console.WriteLine($"SSBO[0] = {result[0]}");
        Assert.That(result[0], Is.EqualTo(42 + 99), "SSBO should contain sum of uSize components");

        // Cleanup
        Rlgl.DisableShader();
        Rlgl.UnloadShaderProgram(compProgram);
        Rlgl.UnloadShaderBuffer(ssbo);
    }
}
