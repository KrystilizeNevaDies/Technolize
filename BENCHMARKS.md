...
`render-snapshot` gate uses (a 1280x720 water body with a stone floor, pillars, and air pockets, lit by
the directional sun plus a coloured point light). Two phases are reported separately:
- **CPU: build SSBO + colour resources**: `WorldShaderResourceBuilder.Build` only — packs the colour
  texture, builds the solid distance field (Euclidean distance transform) used for shadow
  sphere-tracing, and serialises the quadtree (no GPU calls). This is the heavier phase now; it is
  cached per frame, so it only re-runs when the world changes.
...
`GL_TIME_ELAPSED` query — excludes the CPU-side readback):
```bash
dotnet run --project Technolize.Test -c Release -- render-snapshot 30
```

> Lighting uses **hard shadows** (one SDF-sphere-traced ray per light) with an unbounded RGB
> accumulator. The `--sun-rays N` flag and the `render-sun-sweep` command are inert for the current
> output (they drove the old soft-shadow ray averaging) and are kept only for compatibility.
